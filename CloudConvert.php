<?php
namespace BestModules\CloudConvertBundle;

/**
 * @author Andrey Bondar
 */
class CloudConvert extends \CloudConvert
{
    private $apikey;
    private $url;
    private $data;
    ...
    public function getFiles()
    {
        ...
        if ($data->step == 'finished' && isset($data->output)) {
            $files = array();
            
            if (isset($data->output->files)) {
                foreach ($data->output->files as $file) {
                    $files[] = $data->output->url . '/' . $file;
                }
            } else {
                $files[] = $data->output->url . '/' . $data->output->filename;
            }
